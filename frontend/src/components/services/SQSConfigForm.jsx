...
import { SQS_QUEUE_TYPES, calculateSQSCost } from '../../data/sqsData';

const SQSConfigForm = ({ onRemove, onCostUpdate }) => {
    ...
    <Paper elevation={3} sx={{ p: 3, mb: 3 }}>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box display="flex" alignItems="center" gap={2}>
          <Typography variant="h5" fontWeight="bold" color="primary">SQS Configuration</Typography>
          <Chip label="Queues" color="secondary" size="small" />
        </Box>
        <IconButton color="error" onClick={onRemove}><DeleteIcon /></IconButton>
      </Box>

      <Accordion defaultExpanded>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography variant="h6">Queue Configuration</Typography></AccordionSummary>
        <AccordionDetails>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                <InputLabel>Queue Type</InputLabel>
                <Select value={config.queueType} label="Queue Type" onChange={(e) => handleConfigChange('queueType', e.target.value)}>
                  {SQS_QUEUE_TYPES.map((type) => (
            ...
            <Grid item xs={12}>
              <Box sx={{ p: 2, bgcolor: 'info.light', borderRadius: 1 }}>
                <Typography variant="caption" color="info.dark">
                  First 1 million requests per month are free for all queue types
                </Typography>
              </Box>
            </Grid>
          </Grid>
      ...
      <Accordion>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}><Typography variant="h6">Request Volume</Typography></AccordionSummary>
        <AccordionDetails>
          <Grid container spacing={3}>
            <Grid item xs={12} md={6}>
              <TextField fullWidth label="Requests (per month)" type="number" value={config.requests}
                onChange={(e) => handleConfigChange('requests', parseInt(e.target.value) || 0)}
                helperText={config.queueType === 'standard' ? '$0.40 per million' : '$0.50 per million (FIFO)'}
